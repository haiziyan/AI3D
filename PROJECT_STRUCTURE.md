# 项目结构说明

```
AI3D/
│
├── css/                          # 样式文件
│   └── main.css                  # 主样式文件（工程软件配色 + 响应式）
│
├── fonts/                        # 字体文件
│   ├── Consola.ttf
│   ├── Consolas.ttf
│   ├── Papyrus.ttf
│   └── Roboto.ttf
│
├── icon/                         # 图标和图片资源
│   ├── favicon.ico
│   ├── Logo.png
│   └── ...
│
├── js/                           # JavaScript 文件
│   ├── CADWorker/                # CAD 内核 Worker 线程
│   │   ├── CascadeStudioFileUtils.js
│   │   ├── CascadeStudioMainWorker.js
│   │   ├── CascadeStudioShapeToMesh.js
│   │   ├── CascadeStudioStandardLibrary.js
│   │   └── CascadeStudioStandardUtils.js
│   │
│   ├── MainPage/                 # 主页面逻辑
│   │   ├── CascadeMain.js        # 主控制器（已修改：Tab 布局）
│   │   ├── CascadeView.js        # 3D 视图
│   │   └── CascadeViewHandles.js # 3D 交互控制
│   │
│   ├── auth.js                   # ✨ 用户认证和积分管理
│   ├── aiGenerator.js            # ✨ AI 代码生成器
│   ├── config.js                 # ✨ 配置文件（需用户创建）
│   ├── config.example.js         # ✨ 配置模板
│   └── StandardLibraryIntellisense.ts  # TypeScript 类型定义
│
├── node_modules/                 # npm 依赖包
│   ├── @supabase/supabase-js/    # ✨ Supabase 客户端
│   ├── axios/                    # ✨ HTTP 客户端
│   ├── three/                    # Three.js 3D 引擎
│   ├── monaco-editor/            # Monaco 代码编辑器
│   ├── golden-layout/            # 布局系统
│   ├── tweakpane/                # UI 控件
│   ├── opencascade.js/           # CAD 内核
│   └── ...
│
├── test/                         # 测试文件
│   ├── CascadeStudioTest.spec.ts
│   └── ...
│
├── textures/                     # 纹理资源
│   └── dullFrontLitMetal.png
│
├── index.html                    # ✨ 主页面（完全重构）
├── package.json                  # ✨ npm 配置（已更新）
├── package-lock.json             # npm 锁定文件
│
├── .gitignore                    # ✨ Git 忽略规则
├── start.bat                     # ✨ Windows 启动脚本
├── start.sh                      # ✨ Linux/Mac 启动脚本
├── check-config.bat              # ✨ 配置检查脚本
│
├── README_CN.md                  # ✨ 完整中文文档
├── SETUP.md                      # ✨ 配置说明
├── QUICKSTART.md                 # ✨ 快速入门指南
├── AI_PROMPTS.md                 # ✨ AI 提示词示例
├── CHANGELOG.md                  # ✨ 更新日志
├── SUMMARY.md                    # ✨ 项目完成总结
├── PROJECT_STRUCTURE.md          # ✨ 本文件
│
├── LICENSE                       # MIT 许可证
├── manifest.webmanifest          # PWA 配置
├── service-worker.js             # Service Worker
└── CNAME                         # GitHub Pages 配置

✨ = 新增或重大修改的文件
```

## 📁 目录详解

### `/css/` - 样式文件

**main.css** (✨ 完全重写)
- 工程软件配色方案（深蓝灰色系）
- 完整的响应式设计（桌面/平板/手机）
- 现代化的动画效果
- TweakPane 主题定制
- Golden Layout 样式覆盖

### `/js/` - JavaScript 核心代码

#### 新增文件

**auth.js** (✨ 新增)
- `AuthManager` 类：用户认证管理
- Supabase 集成
- 积分系统逻辑
- 用户中心功能
- 交易记录管理

**aiGenerator.js** (✨ 新增)
- `AIGenerator` 类：AI 代码生成
- DeepSeek API 集成
- Token 计费逻辑
- 重试机制
- 错误处理

**config.js** (✨ 需创建)
- Supabase 配置
- DeepSeek API 配置
- 积分系统配置
- 不应提交到 Git

**config.example.js** (✨ 新增)
- 配置文件模板
- 包含详细注释
- 用户复制后填写

#### 修改文件

**MainPage/CascadeMain.js** (✨ 已修改)
- 修改布局配置：从左右分栏改为 Tab 切换
- 使用 `stack` 类型的 Golden Layout
- 保留所有原有功能

### `/node_modules/` - 依赖包

#### 新增依赖
- `@supabase/supabase-js` - 用户认证和数据库
- `axios` - HTTP 请求库

#### 原有依赖
- `opencascade.js` - CAD 内核
- `three` - 3D 渲染引擎
- `monaco-editor` - 代码编辑器
- `golden-layout` - 窗口布局系统
- `tweakpane` - UI 控件库
- `jquery` - DOM 操作

### 根目录文件

#### HTML 文件

**index.html** (✨ 完全重构)
- 新增 AI 输入面板
- 优化的导航菜单
- 用户认证模态框
- 用户中心模态框
- 响应式布局支持
- 引入新的 JS 文件

#### 配置文件

**package.json** (✨ 已更新)
- 更新项目名称和描述
- 添加新依赖
- 添加启动脚本

**.gitignore** (✨ 新增)
- 忽略 `node_modules/`
- 忽略 `js/config.js`（保护密钥）
- 忽略临时文件

#### 脚本文件

**start.bat** (✨ 新增)
- Windows 启动脚本
- 自动检查配置
- 自动安装依赖
- 启动开发服务器

**start.sh** (✨ 新增)
- Linux/Mac 启动脚本
- 功能同 start.bat

**check-config.bat** (✨ 新增)
- 配置检查工具
- 验证配置文件完整性

#### 文档文件

**README_CN.md** (✨ 新增)
- 完整的中文文档
- 功能介绍
- 安装说明
- 使用指南
- API 文档

**SETUP.md** (✨ 新增)
- 详细的配置步骤
- Supabase 设置
- DeepSeek API 获取
- 数据库 SQL 脚本
- 常见问题

**QUICKSTART.md** (✨ 新增)
- 5 分钟快速入门
- 分步骤指导
- 示例项目
- 使用技巧

**AI_PROMPTS.md** (✨ 新增)
- AI 提示词示例
- 最佳实践
- 常见模式
- 进阶技巧

**CHANGELOG.md** (✨ 新增)
- 版本历史
- 功能更新
- 破坏性变更
- 未来计划

**SUMMARY.md** (✨ 新增)
- 任务完成总结
- 功能对比
- 技术栈说明
- 使用流程

**PROJECT_STRUCTURE.md** (✨ 本文件)
- 项目结构说明
- 文件功能描述
- 开发指南

## 🔧 开发指南

### 添加新功能

1. **前端功能**
   - 在 `js/` 目录创建新文件
   - 在 `index.html` 中引入
   - 在 `css/main.css` 中添加样式

2. **AI 功能**
   - 修改 `js/aiGenerator.js`
   - 更新提示词模板
   - 调整计费逻辑

3. **用户功能**
   - 修改 `js/auth.js`
   - 更新 Supabase 数据库结构
   - 添加新的 RLS 策略

### 修改样式

1. **全局样式**
   - 修改 `css/main.css` 中的 CSS 变量
   - 更新颜色方案
   - 调整响应式断点

2. **组件样式**
   - 在 `css/main.css` 中找到对应的类
   - 保持命名一致性
   - 测试响应式效果

### 调试技巧

1. **前端调试**
   - 使用浏览器开发者工具
   - 查看控制台日志
   - 检查网络请求

2. **Worker 调试**
   - 查看 Worker 线程日志
   - 使用 `console.log` 输出
   - 检查消息传递

3. **数据库调试**
   - 使用 Supabase 控制台
   - 查看表数据
   - 检查 RLS 策略

## 📦 部署指南

### 本地开发

```bash
npm install
npm start
```

### 生产部署

1. **静态托管**（推荐）
   - GitHub Pages
   - Netlify
   - Vercel

2. **配置环境变量**
   - 不要提交 `js/config.js`
   - 使用环境变量或构建时注入

3. **优化**
   - 压缩 CSS/JS
   - 启用 CDN
   - 配置缓存策略

## 🔒 安全注意事项

### 敏感文件

**绝对不要提交到 Git:**
- `js/config.js` - 包含 API 密钥
- `.env` - 环境变量文件

**已在 .gitignore 中:**
```
js/config.js
.env
.env.local
```

### API 密钥保护

1. **开发环境**
   - 使用 `config.js` 本地配置
   - 不提交到版本控制

2. **生产环境**
   - 使用环境变量
   - 使用服务器端代理
   - 限制 API 密钥权限

## 📊 文件大小统计

```
总计: ~50MB (主要是 node_modules)

核心文件:
- index.html: ~15KB
- css/main.css: ~25KB
- js/auth.js: ~8KB
- js/aiGenerator.js: ~6KB
- js/MainPage/CascadeMain.js: ~20KB

依赖包:
- node_modules/: ~45MB
- opencascade.js: ~15MB
- three.js: ~5MB
- monaco-editor: ~10MB
```

## 🎯 关键文件速查

| 功能 | 文件位置 |
|------|---------|
| 页面布局 | `index.html` |
| 样式设计 | `css/main.css` |
| Tab 切换 | `js/MainPage/CascadeMain.js` |
| AI 生成 | `js/aiGenerator.js` |
| 用户系统 | `js/auth.js` |
| 配置 | `js/config.js` |
| 3D 渲染 | `js/MainPage/CascadeView.js` |
| CAD 内核 | `js/CADWorker/CascadeStudioMainWorker.js` |

## 📝 维护建议

### 定期更新

1. **依赖包**
   ```bash
   npm update
   npm audit fix
   ```

2. **文档**
   - 保持文档与代码同步
   - 更新版本号
   - 记录重要变更

3. **备份**
   - 定期备份数据库
   - 导出用户数据
   - 保存配置文件

### 监控

1. **API 使用量**
   - DeepSeek API 调用次数
   - Supabase 数据库查询
   - 存储空间使用

2. **用户反馈**
   - GitHub Issues
   - 错误日志
   - 使用统计

---

**项目结构清晰，易于维护和扩展！** 🎉
